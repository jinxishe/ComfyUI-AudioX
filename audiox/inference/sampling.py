...
# ---------------------------------------------------------------------------
# Safe k_diffusion import
#
# Two problems are bypassed here:
#   1. k_diffusion/__init__.py imports evaluation.py, which imports clip,
#      which calls pkg_resources — unavailable in Python 3.12.
#   2. ComfyUI's sys.path may contain a stale k_diffusion.py single-file
#      module that shadows the real k_diffusion package installed in
#      site-packages.
#
# Solution: locate the real package directory via site.getsitepackages(),
# build a lightweight namespace package manually, then load only the four
# submodules needed for inference (utils, layers, external, sampling),
# never touching evaluation.py.
# ---------------------------------------------------------------------------
...
def _import_k_diffusion_safe():
    # Locate the real k_diffusion package directory in site-packages
    k_diff_dir = None
    for sp in site.getsitepackages() + [site.getusersitepackages()]:
        candidate = _os.path.join(sp, "k_diffusion")
        if _os.path.isdir(candidate) and                 _os.path.exists(_os.path.join(candidate, "__init__.py")):
            k_diff_dir = candidate
            break

    if k_diff_dir is None:
        raise ImportError(
            "[AudioX] k_diffusion package directory not found. "
            "Install it with: pip install k-diffusion==0.1.1.post1"
        )

    # Register a lightweight namespace — do NOT exec __init__.py
    pkg = types.ModuleType("k_diffusion")
    pkg.__path__    = [k_diff_dir]
    pkg.__package__ = "k_diffusion"
    pkg.__spec__    = importlib.machinery.ModuleSpec("k_diffusion", None)
    sys.modules["k_diffusion"] = pkg

    # Load only the submodules required for inference
    for sub in ("utils", "layers", "external", "sampling"):
        sub_path = _os.path.join(k_diff_dir, f"{sub}.py")
        if not _os.path.exists(sub_path):
            continue
        spec = importlib.util.spec_from_file_location(
            f"k_diffusion.{sub}", sub_path,
