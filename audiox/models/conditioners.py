...
class CLIPConditioner(Conditioner):
    ...
    def __init__(
        ...
        if self.clip_model_name=='clip-vit-base-patch32':
            ...
            # 优先从本地加载，回退到 HuggingFace
            import os as _os
            _local = _os.environ.get('AUDIOX_CLIP_MODEL_PATH', '')
            if _local and _os.path.isdir(_local):
                self.visual_encoder_model = CLIPVisionModelWithProjection.from_pretrained(_local, local_files_only=True)
            else:
